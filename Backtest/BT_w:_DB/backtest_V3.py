...
target_weights.rename(columns = {'Unnamed: 0':'date'}, inplace = True)
target_weights.set_index(keys = 'date', inplace = True)

ticker_list = list(target_weights.index)
...
price_data = database_query.get_price_data(tick_list = ticker_list, 
                                           database = db_name, 
                                           conn = con)
