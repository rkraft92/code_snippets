...
class simulation:
    ...
    def friedman_model(self):
        ...
        epsilon = self.random_state.normal(0, self.sigma, size=(self.n_obs, self.n_sim))
        
        for m in range(0, self.n_sim):
            
            self.y_act[:,m] = self.f_friedman + epsilon[:,m]
        
        return self.X, self.y_act, self.f_friedman
    
    
    def linear_model(self):
        self.f_linear = sum((j+1) * self.X[:,j] for j in range(0,5))
        self.epsilon = self.random_state.normal(0, self.sigma, size=(self.n_obs, n_sim))
        
        for m in range(0, self.n_sim):
    
            self.y_act[:,m] = self.f_linear + self.epsilon
        
        return self.X, self.y_act, self.f_linear
